...
bNode = bcNode("172.29.42.250")

#Initializing P2P Node
node = Node("172.29.42.250",4444,bNode, npeer=10)
...
#-----------------------------------------------------------------------
    ...
    if  not node.bNode.exists:
        ...
        thread.start()
        tosend='-'.join(["172.29.42.250",str(port)])
        ...
        if node.loadData():
            ...
            thread=threading.Thread(target = node.connectionSpawner, args = [], daemon = True)
            thread.start()
            tosend='-'.join([str(node.guid),"172.29.42.250",str(port)])
